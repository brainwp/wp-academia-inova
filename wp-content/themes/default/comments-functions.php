...
function theme_comment_form()
{
	...
	if($_POST["name"]!="" && $_POST["name"]!=__("Your name", $themename) && $_POST["email"]!="" && $_POST["email"]!=__("Your email", $themename) && preg_match("#^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,4})$#", $_POST["email"]) && $_POST["message"]!="" && $_POST["message"]!=__("Message", $themename))
	{
		...
		$data = array(
			...
			'comment_author_url' => ($values['website']!=__("Website (optional)", $themename) ? $values['website'] : ""),
		...
		{
			$result["submit_message"] = __("Your comment has been added", $themename);
			//get post comments
			//post
		...
		{
			$result["isOk"] = false;
			$result["submit_message"] = __("Error while adding comment", $themename);
		}
	}
	else
	{
