...
function theme_slider($atts, $content) {
	...
	$slider_image_url[0] = $theme_options["slider_image_url"];
	$slider_image_title[0] = $theme_options["slider_image_title"];
	$slider_image_subtitle[0] = $theme_options["slider_image_subtitle"];
	$slider_image_link[0] = $theme_options["slider_image_link"];
	...
	if(  count( $slides_count ) ) {
		...
		for( $i=0; $i<$slides_count; $i++ ) {
			foreach ($slider_image_url as $key => $urldaimagem) {
				$output .= '<li id="slide_' . $i . '" style="background-image: url(' . $urldaimagem[$i] . ');"></li>';
			}
		}
		$output .= '</ul>';
	}
		...
		for ( $i=0; $i<$slides_count; $i++ ) {
			if( $theme_options["slider_image_title"][$i] != "" || $theme_options["slider_image_subtitle"][$i] != "") {
				...
				if($theme_options["slider_image_title"][$i]!="")
					$output .= '<h1 class="title"><a href="' . esc_attr($theme_options["slider_image_link"][$i]) . '">' . esc_attr($theme_options["slider_image_title"][$i]) . '</a></h1>';
